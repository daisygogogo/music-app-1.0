...
const HOT_TITLE = "热门";
const ANCHOR_HEIGHT = 18.6; //右侧快速入口，每一项之间的距离，自己根据布局计算而来
...
export default {
  ...
  methods: {
    ...
    onShortCutTouchStart(e) { //点击右侧菜单，滚动到相应的位置
    ...
    getListHeight(){
      let listHeight = [];
      listHeight.push(0)
      for(let i=0; i < this.$refs.singerGroup.length-1; i++){
        listHeight.push(this.$refs.singerGroup[i+1].offsetTop)
      }
    ...
    _getCurrentIndex(val){
      for(let i=0; i < this.listHeight.length; i++){
        console.log(this.listHeight[i],val, this.listHeight[i + 1])
        if(this.listHeight[i] <= val && val < this.listHeight[i + 1] && i !== this.listHeight.length-1){
          this.currentIndex = i;
        }else if(i === this.listHeight.length-1 && this.listHeight[i] <= val){
